...
celltype=data.frame(ClusterID=0:19,
                    celltype='unkown')
celltype[celltype$ClusterID %in% c(0,15),2]='Non-tumor1'
celltype[celltype$ClusterID %in% c(10),2]='LC-meta'
celltype[celltype$ClusterID %in% c(1,2,7,14,18),2]='Non-tumor2'
celltype[celltype$ClusterID %in% c(3,4,5,6,8,9,11,12,13,16,17,19),2]='Glioma'
...
saveRDS(sce,file = "GSE117891_注释后.rds")
...
p.dim.cell=DimPlot(sce, reduction = "tsne", group.by = "celltype",label = T,pt.size = 1,repel = T) 
p.dim.cell
ggsave(plot=p.dim.cell,filename="ADAM17_umap_Gliama_6clu_注释.pdf",width=6, height=4)


sce <- RunTSNE(sce, reduction = "pca", dims = 1:25)
p.dim.cell=DimPlot(sce, reduction = "tsne", group.by = "celltype",label = T,pt.size = 1) 
p.dim.cell
ggsave(plot=p.dim.cell,filename="ADAM17_tsne_Gliama_6clu_注释.pdf.pdf",width=6, height=4)
...
if (!gene_name %in% rownames(Ast_sce)) {
  stop("基因不存在于数据中！")
}
